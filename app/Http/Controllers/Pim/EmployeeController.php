...
class EmployeeController extends Controller
{
    ...
    public function getEmployeeStatus($id){
        ...
        if(count($data) == 0){
            $emp_type_data = UserEmployeeTypeMap::where('user_id', $id)->first();
             ...
             UserEmployeeStatusMap::create([
                'user_id' => $id,
                'employee_status_id' => 1,
                'from_date' => $emp_type_data->from_date,
                'remarks' => "Generated by System.",
            ]);
        }

        return UserEmployeeStatusMap::where('user_id', $id)->get();
    }
    ...
    public function updateEmployeeStatus(Request $request){
        ...
        $oldData = UserEmployeeStatusMap::where('user_id', $user_id)->orderBy('id', 'DESC')->first();

        $this->validate($request, [
            'status_name' => 'required',
            'status_eff_date' => 'required|after:'.$oldData->from_date,
        ],[
        ...
        try{
            if(strtotime($status_eff_date) <= strtotime($current_date)){
                ...
                User::where('id', $user_id)->update(['status' => $status_name]);
            }

            $oldData->to_date = $prev_date;
            $oldData->save();

            UserEmployeeStatusMap::create([
                'user_id' => $user_id,
                'employee_status_id' => $status_name,
                'from_date' => $status_eff_date,
